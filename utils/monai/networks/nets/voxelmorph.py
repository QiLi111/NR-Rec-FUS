...
class VoxelMorph(nn.Module):
    ...
    def forward(self, moving: torch.Tensor, fixed: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.ddf_dirc == 'Fix':
            return self.warp(moving, x), x  # warp moving image
        elif self.ddf_dirc == 'Move':
            return self.warp(fixed, x), x  # warp fixed image
        else:
            raise('not a valid DDF direction')
